  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm leading-relaxed text-[#8899b8]">
        {result === 'authentic'
          ? `All forensic checks passed. ELA integrity: ${analysisResult?.ela_integrity_score?.toFixed(1)}%. No typography or compression anomalies detected.`
          : refExists
            ...
            : analysisResult?.forged_fields?.length > 0
              ...
              : `Error Level Analysis detected pixel compression inconsistencies. ELA integrity: ${analysisResult?.ela_integrity_score?.toFixed(1)}% (noise score: ${analysisResult?.ela_noise_score?.toFixed(1)}/100).`}
      </p>
